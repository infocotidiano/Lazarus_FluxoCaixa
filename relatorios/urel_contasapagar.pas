...
type
  ...
  Tfrm_RelContasAPagar = class(TForm)
    ...
    procedure bdDetalheAPagarAfterPrint(Sender: TObject);
...
procedure Tfrm_RelContasAPagar.bdDetalheAPagarAfterPrint(Sender: TObject);
begin
...
begin
  ...
  TDataSet(Relatorio.DataSource.DataSet).FieldByName('valor').AsFloat;
...
procedure Tfrm_RelContasAPagar.bdDetalhePagasBeforePrint(Sender: TObject;
  var PrintIt: Boolean);
begin
  PrintIt := TDataSet(Relatorio.DataSource.DataSet).
          FieldByName('situacao').AsString = 'B';

  FAgrupadoPagas := FAgrupadoPagas +
  TDataSet(Relatorio.DataSource.DataSet).FieldByName('valorpago').AsFloat;
...
begin
  ...
  rlPagasAgrupado.Caption:= FormatFloat('###,##0.00',FAgrupadoPagas);
